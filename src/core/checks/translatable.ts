...
/**
 * An address such as `http://…`: letters in it are no words. The scheme starts only where no scheme character is
 * before it: otherwise every letter of a long word would start a new attempt, in quadratic time.
 */
const URL = /(?<![a-z0-9+.-])[a-z][a-z0-9+.-]*:\/\/\S*/gi;
